    ...
    *,
    ...
    for lead in leads:
        ...
        if ai_data and isinstance(ai_data, dict):
            ...
            # Lead Quality Score 0-100 — override deterministic kalau AI valid
            ai_q = sanitize_ai_quality_score(ai_data.get("quality_score"))
            if ai_q is not None:
                lead.quality_score = ai_q
            # BI summary — AI kalau ada, else fallback deterministic
...
def _build_system_prompt(leads: list[QualifiedLead]) -> str:
    ...
    return (
        ...
        "buying signals (active ads, revenue band). Use the provided base_score as "
        "an anchor and adjust within ~15 points.\n"
...
def _parse_json_response(text: str) -> dict[str, dict[str, Any]]:
    ...
    for domain, payload in results.items():
        ...
        # quality_score is optional & may be missing/invalid — keep raw for caller
