...
from src.email_verifier import (
    best_status as _best_status,
    verify_emails as _verify_emails,
)
...
async def enrich_extras(
    ...
    *,
    ...
    enable_bi: bool = True,  # zero-budget HTML BI — default ON
    enable_email_verify: bool = False,  # default OFF — adds latency (SMTP probe)
    email_verify_use_providers: bool = True,  # use free-tier APIs if keys set
) -> dict:
    ...
    Return: dict dengan key:
        ...
        employee_range / location_count / founded_year / years_in_business /
        social_profiles / tech_signals / bi_score  (Business Intelligence)
    """
    ...
    # 1b. Email Verification (SMTP RCPT + free-tier provider API fallback)
    if enable_email_verify and result["emails_found"]:
        try:
            # SMTP probe is blocking I/O — run in thread to keep event loop responsive.
            verifications = await asyncio.to_thread(
                _verify_emails,
                result["emails_found"],
                use_smtp=True,
                use_providers=email_verify_use_providers,
                delay_between=True,
            )
            statuses_map = {v["email"]: v["status"] for v in verifications}
            methods = {v["method"] for v in verifications if v.get("method")}
            result["email_statuses"] = statuses_map
            result["best_email_status"] = _best_status(list(statuses_map.values()))
            # method = strongest non-"none" method used
            method_order = ("zerobounce", "myemailverifier", "hunter", "smtp", "format", "none")
            for m in method_order:
                if m in methods:
                    result["email_verification_method"] = m
                    break
        except Exception as e:  # noqa: BLE001
            print(f"[extras] {domain} verify fail: {type(e).__name__}: {e}")
    ...
    # 5. Business Intelligence enrichment (zero-budget HTML parse)
    if enable_bi and base_html:
        try:
            bi = enrich_business_intelligence(base_html, domain)
            result.update(bi)
        except Exception as e:  # noqa: BLE001
            print(f"[extras] {domain} BI fail: {type(e).__name__}: {e}")
...
async def enrich_extras_batch(
    ...
    *,
    ...
    print(
        ...
        f"ads={enable_ads}, competitors={enable_competitors}, bi={enable_bi}, "
        f"email_verify={enable_email_verify})"
    )
