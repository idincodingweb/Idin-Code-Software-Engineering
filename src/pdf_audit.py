...
def _generate_one(lead, output_dir: str) -> str:
    ...
    social_profiles = getattr(lead, "social_profiles", []) or []
    tech_signals = getattr(lead, "tech_signals", []) or []
    bi_score = int(getattr(lead, "bi_score", 0) or 0)
    if bi_summary or employee_range or tech_signals or social_profiles:
        story.append(Paragraph("Business Intelligence", h2))
        if bi_summary:
            story.append(Paragraph(_safe(bi_summary), body))
        bi_data = [
            ["BI Sophistication", f"{bi_score} / 100"],
            ["Employee Range", _safe(employee_range or "unknown")],
            ["Locations (est.)", str(location_count)],
            ["Founded", str(founded_year) if founded_year else "—"],
            ["Social Profiles", _safe(", ".join(social_profiles[:6]) or "—")],
            ["Tech Stack", _safe(", ".join(tech_signals[:6]) or "—")],
        ]
        story.append(_build_table(bi_data))
