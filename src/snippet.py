...
Sekarang analyst bersifat config-driven per niche:
- prompt context di-load dari YAML
- fallback reasons/outreach rule di-load dari YAML
- niche baru cukup tambah file config, tanpa ubah kode Python
"""
...
def _build_system_prompt(leads: list[QualifiedLead]) -> str:
    ...
    return (
        ...
        f"performance data to identify SALES OPPORTUNITIES that agencies can pitch.\n\n"
        f"{analyst['focus']}\n\n"
        ...
        "   - gold_reasons (1-2 sentences): WHY this is a hot lead. Reference specific gaps with business impact.\n"
        "   - outreach_angle (1 sentence): A cold email subject line OR opening hook an agency can use immediately.\n"
        "   - quality_score (integer 0-100): how good a SALES PROSPECT this lead is for an agency. Use the provided base_score as an anchor and adjust within ~15 points.\n"
...
def _build_user_prompt(leads: list[QualifiedLead]) -> str:
    lines = [
        "Analyze these businesses. For each, generate gold_reasons, outreach_angle, quality_score (0-100) & bi_summary. Return JSON only.\n",
        "Data per business:",
    ]
    ...
    for lead in leads:
        ...
        if lead.meta_pixel_in_html:
            pixels.append("Meta")
        if getattr(lead, "tiktok_pixel_in_html", False):
...
def _template_context(lead: QualifiedLead) -> dict[str, Any]:
    ...
    label = brand or lead.domain.replace("www.", "").split(".")[0].title()
    ...
    return {
        ...
        "missing_tracking_items": ", ".join(missing[:4]) if missing else "none",
        "social_profiles_count": len(social),
        "social_profiles": ", ".join(social[:5]) if social else "NONE",
        "tech_signals_count": len(tech),
        "tech_signals": ", ".join(tech[:5]) if tech else "NONE",
